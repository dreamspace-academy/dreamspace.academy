<!-- Content -->
<div class="template-content">

	<!-- Funding Partners -->
  <!-- Header and subheader -->
  <div class="template-component-header-subheader">
    <h2>Our Funding Partners</h2>
    <h6></h6>
    <div></div>
  </div>

  <!-- Section -->
  <div class="template-content-section template-padding-bottom-5 template-main">

    <!-- Gallery -->
    <div class="template-component-gallery">
      <ul class="template-layout-25x25x25x25 template-clear-fix">
        <li class="template-layout-column-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/bookbridge-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>BookBridge</h6>
              </br>
              <span>Swiss</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://www.bookbridge.org/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
				<li class="template-layout-column-center-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/aracreate-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>araCreate</h6>
              </br>
              <span>Germany</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="http://aracreate.com/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
				<li class="template-layout-column-center-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/sylc-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>SYLC</h6>
              </br>
              <span>Sri Lanka</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://sylcsl.org/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
				<li class="template-layout-column-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/open-edge-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>OpenEdge</h6>
              </br>
              <span>UK</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://www.openedge.org.uk/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
      </ul>
    </div>
  </div>

	<!-- Project Partners -->
	<!-- Header and subheader -->
	<div class="template-component-header-subheader">
		<h2>Our Project Partners</h2>
		<h6></h6>
		<div></div>
	</div>

	<!-- Section -->
	<div class="template-content-section template-padding-bottom-5 template-main">

		<!-- Gallery -->
		<div class="template-component-gallery">
			<ul class="template-layout-25x25x25x25 template-clear-fix">
				<li class="template-layout-column-left">
					<div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
						<img src="../../media/images/about/partners/siwi-dreamspace.jpg" alt="dreamspace-partners" />
						<div>
							<h6>Stockholm International Water Institute (SJWP)</h6>
							</br>
							<span>Sweden</span>
						</div>
						<div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
							<ul>
								<li><a target="_blank" href="https://www.siwi.org/" class="template-component-social-icon-rss"></a></li>
							</ul>
						</div>
					</div>
				</li>
				<li class="template-layout-column-center-left">
					<div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
						<img src="../../media/images/about/partners/duke-dreamspace.jpg" alt="dreamspace-partners" />
						<div>
							<h6>The Duke of Edinburgh's International Award</h6>
							</br>
							<span>UK</span>
						</div>
						<div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
							<ul>
								<li><a target="_blank" href="https://intaward.org/" class="template-component-social-icon-rss"></a></li>
							</ul>
						</div>
					</div>
				</li>
				<li class="template-layout-column-center-right">
					<div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
						<img src="../../media/images/about/partners/s4ig-dreamspace.jpg" alt="dreamspace-partners" />
						<div>
							<h6>Skills for Inclusive Growth (S4IG)</h6>
							</br>
							<span>Australia</span>
						</div>
						<div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
							<ul>
								<li><a target="_blank" href="https://inclusivegrowth.com.lk/" class="template-component-social-icon-rss"></a></li>
							</ul>
						</div>
					</div>
				</li>
				<li class="template-layout-column-right">
					<div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
						<img src="../../media/images/about/partners/stsa-dreamspace.jpg" alt="dreamspace-partners" />
						<div>
							<h6>Sri Lankan Tamil Student Association (STSA)</h6>
							</br>
							<span>USA</span>
						</div>
						<div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
							<ul>
								<li><a target="_blank" href="https://www.stsachicago.com/" class="template-component-social-icon-rss"></a></li>
							</ul>
						</div>
					</div>
				</li>
			</ul>
		</div>
	</div>

  <!-- Network Partners -->
  <!-- Header and subheader -->
  <div class="template-component-header-subheader">
    <h2>Our Network Partners</h2>
    <h6></h6>
    <div></div>
  </div>

  <!-- Section -->
  <div class="template-content-section template-padding-bottom-5 template-main">

    <!-- Gallery -->
    <div class="template-component-gallery">
      <ul class="template-layout-25x25x25x25 template-clear-fix">
        <li class="template-layout-column-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/motionlab-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>MotionLab Berlin</h6>
              </br>
              </br>
              <span>Germany</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://motionlab.berlin/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-center-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/gig-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Global Innovation Gathering (GIG)</h6>
              </br>
              <span>Germany</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://www.globalinnovationgathering.org/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-center-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/cpbr-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Centre for Peacebuilding and Reconciliation (CPBR)</h6>
              </br>
              <span>Sri Lanka</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://www.peaceinsight.org/conflicts/sri-lanka/peacebuilding-organisations/cpbr/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/un-volunteers-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>UN Volunteers</h6>
              </br>
              </br>
              <span>Sri Lanka</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="http://unvlk.org/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/greenbuzz-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>GreenBuzz</h6>
              </br>
              </br>
              <span>Germany</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://greenbuzzberlin.de/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-center-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/troublemaker-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Troublemaker Shenzhen</h6>
              </br>
              </br>
              <span>China</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://troublemaker.site" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-center-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/steam-head-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>SteamHead</h6>
              </br>
              </br>
              <span>USA | China</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://steamhead.space/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/gosh-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Gathering for Open Science Hardware (GOSH)</h6>
              </br>
              <span>Global</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="http://openhardware.science/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/shdb-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Science Hack Day Berlin</h6>
              </br>
              </br>
              <span>Germany</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="http://berlin.sciencehackday.org/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-center-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/scout-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Scout Association</h6>
              </br>
              </br>
              <span>Sri Lanka</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://en.wikipedia.org/wiki/Sri_Lanka_Scout_Association" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-center-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/lwb-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Lecturers Without Borders</h6>
              </br>
              <span>France</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="https://scied.network/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-right">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/jogl-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Just One Giant Lab</h6>
              </br>
              </br>
              <span>France</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="http://jogl.io/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>
        <li class="template-layout-column-left">
          <div class="template-component-image template-component-image-hover-slide-enable template-fancybox template-preloader">
            <img src="../../media/images/about/partners/a2p-dreamspace.jpg" alt="dreamspace-partners" />
            <div>
              <h6>Access 2 Perspectives</h6>
              </br>
              <span>Germany</span>
            </div>
            <div class="template-component-social-icon template-component-social-icon-style-1 template-clear-fix">
              <ul>
                <li><a target="_blank" href="http://access2perspectives.com/" class="template-component-social-icon-rss"></a></li>
              </ul>
            </div>
          </div>
        </li>

      </ul>
    </div>
  </div>

</div>
