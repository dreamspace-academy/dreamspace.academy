<?php include('0-0-header.php'); ?>

<!-- Content -->
<div class="template-content">
	<!-- Main -->
	<div class="template-content-section template-main">
		<!-- Header and subheader -->
		<div class="template-component-header-subheader">
			<h2>Support Us</h2>
			<h6>Join our mission to co-create a sustainable impact</h6>
			<div></div>
		</div>

		<div class="template-component-divider template-component-divider-style-1"></div>

		<!-- Opencollective Donation -->
		<iframe src="https://opencollective.com/embed/dreamspace-academy/donate" style="width: 95%; min-height: 110vh;"></iframe>

		<!-- Header and subheader -->
		<div class="template-component-header-subheader">
			<h2></h2>
			<h6>Check our support deck to know more about our needs and how you can support us !!! </br>
				<a href="http://supportdeck.dreamspace.academy">http://supportdeck.dreamspace.academy</a>
			</h6>
			<div></div>
		</div>

		<!-- Support deck -->
		<div class="template-content-section template-padding-top-reset">
			<iframe src="https://files.aravinth.info/decks/dreamspace-academy-support-deck.pdf#page=3&view=fit&toolbar=0" type="application/pdf" height=500px width="100%" class="responsive"></iframe>
		</div>

		<div class="template-component-feature template-component-feature-style-1 template-component-feature-position-left template-component-feature-size-medium">
			<!-- Layout 33x33x33 -->
			<ul class="template-layout-33x33x33 template-clear-fix">

				<li class="template-layout-column-left">
					<div class="template-icon-feature" style="text-align:center; padding:15px;">
						<div style="font-size:xx-large; color: White">
							<i class="fas fa-hand-holding-usd"></i>
						</div>
					</div>
					<h5>Send your financial donations to </h5>
					<p>
						<b>Account Name:</b></br>DreamSpace</br>
						<b>Bank Name:</b></br>Bank of Ceylon</br>
						<b>Bank Address:</b></br>426 - Kallady Branch, Batticaloa 30000, Sri Lanka</br>
						<b>Account Number:</b></br>84375801</br>
						<b>SWIFT Code:</b></br>BCEYLKLX</br>
					</p>
				</li>

				<li class="template-layout-column-center">
					<div class="template-icon-feature" style="text-align:center; padding:15px;">
						<div style="font-size:xx-large; color: White">
							<i class="fas fa-tools"></i>
						</div>
					</div>
					<h5>Send your material donations to </h5>
					<p>
						DreamSpace Academy</br>
						7A, Saravana Road, Kallady,</br>
						Batticaloa 30000,</br>
						Sri Lanka
					</p>
				</li>

				<li class="template-layout-column-right">
					<div class="template-icon-feature" style="text-align:center; padding:15px;">
						<div style="font-size:xx-large; color: White">
							<i class="fas fa-hands-helping"></i>
						</div>
					</div>
					<h5>Send your opportunites to </h5>
					<p>
						Email: <a href="mailto:ara@dreamspace.academy">ara@dreamspace.academy</a>
						</br><a href="mailto:kishoth@dreamspace.academy">kishoth@dreamspace.academy</a>
					</p>
				</li>

			</ul>
		</div>

		<!-- Feature -->
		<div class="template-content-section template-padding-top-reset">
			<p>
				<center>
					Check our support deck below to know more about our needs and how you can support us !!!
				</center>
			</p>
			<embed src="https://files.aravinth.info/decks/dreamspace-academy-support-deck.pdf#page=3&view=fit&toolbar=0" type="application/pdf" height=500px width="100%" class="responsive">
		</div>

		<div class="template-content-section template-padding-top-reset">
			<center>
				<iframe src="https://opencollective.com/embed/dreamspace-academy/donate?useTheme=false" style="width: 95%; min-height: 150vh;" />
			</center>
		</div>

	</div>
</div>

<?php include('9-0-footer.php'); ?>
